    ...
    if (isDownloaded && indexedSongs) {
      ...
      if (!found) {
        ...
        const result = {
          ...
          cover: song.attributes?.cover?.data[0]?.attributes.url,
          date_de_sortie: song.attributes?.date_de_sortie,
          artist: song.attributes.artist,
          album: song.attributes.album,
          pays: song.attributes.pays,
          ville: song.attributes.ville,
        };
        addSongs(result);
      }
    }
  }, [isDownloaded, indexedSongs]); // Dépendances pour surveiller les changements
