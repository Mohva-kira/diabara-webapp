...
const MusicPlayer = ({ setIsVisible }) => {
  ...
  const handlePlayPause = async () => {
    try {
      // Vérifiez si une chanson est active

      // Envoi des données à Google Analytics
      ReactGA.send({
        hitType: "pageview",
        page: window.location.href,
        title: activeSong.attributes?.name || "Chanson inconnue",
      });
      ...
      console.error("Erreur lors de l'envoi des données played :", error);
    }
  };
  ...
  return (
    <div className="relative sm:px-2 px-8  w-full flex items-center justify-between">
      <div
